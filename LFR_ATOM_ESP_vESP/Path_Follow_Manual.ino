void path_follow()
{
    ...
    while (digitalRead(btn_m) && digitalRead(btn_l) && digitalRead(btn_r) && digitalRead(btn_d))
    {
    point:
        sensor_read();
        check_inverse();
        ...
        else if (sum == 8)
        {
            ...
            while (sum == 8)
            {
                ...
                if (m2 - m1 > finish_timer)
                {
                    ...
                    text("PathFollow", 04, 3);
                    digitalWrite(light, 0);
                    if (digitalRead(btn_m) && digitalRead(btn_l) && digitalRead(btn_r) && digitalRead(btn_d))
                    {
                        delay(1000);
                        goto point;
                    }
